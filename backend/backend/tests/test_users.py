...
def test_cookie_login(client):
    ...
    response = client.get(
        "/api/users/me",
        cookies={"ciasteczkowy_potwor": response.cookies["ciasteczkowy_potwor"]}
    )
    assert response.status_code == 200
...
def test_delete_file(client):
    ...
    response = client.delete(
        f"/api/files/delete/{file_id}",
        headers=pytest.shared["token"]
    )
    assert response.status_code == 204
    assert os.path.exists(os.path.join(os.environ["FILE_PATH"], file_id)) == False
